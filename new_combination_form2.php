...
if(isset($_POST['submit']))		{	
				...
				for($i = 0; $i < count($subject_id); $i++){
					$sql4="select * from $db_name.subject WHERE subject_id= ".$subject_id[$i];
					$result4 = mysqli_query($con,$sql4);
					while($row4 = mysqli_fetch_array($result4)){	
						$name = $row4['subject_name'];
						//$numeric_name=$row4['subject_id'];
						if($row4['code']=='P245'){$name[0]='D'; $combination_name.=$name[0];} //CRE is represented by D
						else {$combination_name.=$name[0];} // generate combination name from first letters of subjects
					}
				}
				
				
				$sql6="select *  from $db_name.combination where $db_name.combination.combination_name='$combination_name'"; //CHECK TO  SEETHAT combination DOESNOT EXIST
				$result6 = mysqli_query($con,$sql6);
					
					if(mysqli_num_rows($result6)>=1){
					$message="Failed! A Combination with name: $combination_name already exists"; //Do not insert duplicates
					}//if(mysqli_num_rows	
					
					else{ //ok to insert
				
				
						$sql="INSERT INTO $db_name.combination (combination_name) VALUES( '$combination_name')"; //insert 
