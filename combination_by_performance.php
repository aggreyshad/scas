...
if(isset($_POST['getcombination']))		{	
		...
		if(mysqli_num_rows($result5)<=0){ $message="There are no Combinations registered!"; /*die ("There are no Combinations registered!");*/}						 
			...
			while($row5 = mysqli_fetch_array($result5)){ // while there are combinations in db
				...
				if(!$result6){ $message="Failed to select subjects corresponding to Combination <b>".$row5['combination_name']."</b>"; }
				...
				else { //shows subjects corresponding to a given combination
					...
					$sql8="select * from $db_name.marks, $db_name.student, $db_name.subject,  $db_name.exams, $db_name.class WHERE $db_name.marks.student_id=$db_name.student.student_id AND $db_name.marks.subject_id=$db_name.subject.subject_id AND $db_name.marks.exam_id=$db_name.exams.exam_id AND $db_name.marks.class_id=$db_name.class.class_id AND $db_name.marks.exam_id=4 AND $db_name.student.student_id=$studentId";
						...
						else{ // there are marks for student
						
							while($row8 = mysqli_fetch_array($result8)){ // while there are marks for student
								
								$current_subject_id=$row6['subject_id'];
								echo " <br /><br /> Curent A L: ".$current_subject_id." ". $row6['subject_name']. "";
								$current_aggregate_id=$row8['aggregate_id'];
						  		$sql7="select * from $db_name.subject, $db_name.subjectprerequisites WHERE $db_name.subject.subject_id=$db_name.subjectprerequisites.o_level_subject_id  AND $db_name.subjectprerequisites.subject_id=$current_subject_id AND $db_name.subjectprerequisites.aggregate_id <= $current_aggregate_id"; 									
								//above querry compares student performance with prerequisites
						  		...
						  		if(!$result7){ $message="Failed to select all o level prerequisites for A subject ".$row6['subject_name'];}
								...
								else{ // there are O level prerequisites
									...
									while($row7 = mysqli_fetch_array($result7)){ // while there are o level prerequisites
										...
										//if(($current_aggregate_id >= 1) && ($current_aggregate_id <= $row7['aggregate_id'])){ 
										...
										elseif($row7['type']==1){ 
											...
											$count_required +=1; 
											echo "Sub: <b>".$row7['subject_name']."</b> Min: <b>".$current_aggregate_id."</b><br />";
										}
										elseif($row7['type']==2){ 
											$has_required=true; 
											$passed_required=true; 
								...
								} // close else there are O level prerequisites
								
								
								
							} // close while there are marks for student
							
							
							
						} // close else there are marks for student
						
						
					} // close // while there are subjects corresponding to given combination
					
				} // close else //shows subjects corresponding to a given combination
				
									//***********************************************
									if($has_required=true && $count_required >=3){
		...
		} // close else //shows combinations in the db
		
	} //close //if there is a request to get a combination
?>
<?php 
