...
$sql="select * from $db_name.marks, $db_name.student, $db_name.subject,  $db_name.exams, $db_name.class WHERE $db_name.marks.student_id=$db_name.student.student_id AND $db_name.marks.subject_id=$db_name.subject.subject_id AND $db_name.marks.exam_id=$db_name.exams.exam_id AND $db_name.marks.class_id=$db_name.class.class_id AND $db_name.marks.exam_id=1 GROUP BY $db_name.marks.student_id";  // exam_id 4 rep UCE
		...
		<div class="row">
            <div class="col-xs-12 col-md-12 col-lg-12">
              <div class="box box-solid box-default">
                <div class="box-header with-border">
                  <h3 class="box-title"><?php //echo $row['exam_name']; ?>BOT Results</h3>
                </div><!-- /.box-header -->
                <div class="box-body table-responsive">		
...
-->                        <div class="pull-left">
						...
						<?php 
						//$array_of_aggregates = $row['student_id']; // will generate uniques aray name for each student
						//$array_of_aggregates = '$array_of_aggregates';
						
						
						while($row3 = mysqli_fetch_array($result3)){?>
                          <tr>
                            <td><?php printf($row3['subject_name']); ?></td>
                            <td><?php printf($row3['mark']); ?></td>
                            <td><?php printf($row3['aggregate_id']); ?></td>
                            <?php $array_of_aggregates[] = $row3['aggregate_id'];// create array of all aggregates?>
                          </tr>
                          
                        <?php } // while($row3 ?>
                        <tfoot>
                          <tr>
                            <td colspan="3"><b>Grade Aggregate:</b> 
							<?php if(isset($array_of_aggregates)){ echo best_eight($array_of_aggregates); ?>
                            <small  class="label bg-yellow" title="<?php echo best_eight($array_of_aggregates); ?> Aggregates in 8 Best Done Subjects">(Best 8)</small>
                            <?php  unset($array_of_aggregates);  } ?>
                            </td>
                           </tr>
                           <tr>
                            <td colspan="3">
                            <a href="?studentId=<?php echo $row['student_id']; ?>" class="button button-primary pull-right" title="Get Combination for - <?php echo ucwords(strtolower($row['student_surname']." ".$row['student_othernames'])); ?>"> Get Combination <i class="fa fa-arrow-circle-right" style="color:#33CC66;"></i></a>
                            </td>
                          </tr>
                        </tfoot>
                        </table>
